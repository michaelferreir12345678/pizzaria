...
const usuariosController = {
    ...
    fazerLogin: (req,res) => {
        ...
        const meuUsuario = objeto.usuarios.find(usuario => usuario.email === req.body.usuario) //Verificação do usuário
        if(!meuUsuario) {
            return res.send('Usuário ou senhas inválidos');
        }

        const senhaEstaCorreta = bcrypt.compareSync(req.body.senha, meuUsuario.senha) //Verificação da senha
        if(!senhaEstaCorreta) {
            return res.send('Usuário ou senhas inválidos');
        }
